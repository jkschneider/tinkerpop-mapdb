...
public class MapdbGraphStep<S extends Element> extends GraphStep<S> {
    public final List<HasContainer> hasContainers = new ArrayList<>();

    @SuppressWarnings({"unchecked", "RedundantCast"})
    public MapdbGraphStep(final GraphStep<S> originalGraphStep) {
        super(originalGraphStep.getTraversal(), originalGraphStep.getGraph(MapdbGraph.class), originalGraphStep.getReturnClass(), originalGraphStep.getIds());
    ...
    private Iterator<? extends Edge> edges() {
        ...
        if (this.ids != null && this.ids.length > 0)
            return this.iteratorList(this.getGraph(MapdbGraph.class).iterators().edgeIterator(this.ids));
        else
            return null == indexedContainer ?
                    this.iteratorList(this.getGraph(MapdbGraph.class).iterators().edgeIterator()) :
                    MapdbHelper.queryEdgeIndex(this.getGraph(MapdbGraph.class), indexedContainer.key, indexedContainer.value).stream()
    ...
    private Iterator<? extends Vertex> vertices() {
        ...
        // ids are present, filter on them first
        if (this.ids != null && this.ids.length > 0)
            return this.iteratorList(this.getGraph(MapdbGraph.class).iterators().vertexIterator(this.ids));
        else
            return null == indexedContainer ?
                    this.iteratorList(this.getGraph(MapdbGraph.class).iterators().vertexIterator()) :
                    MapdbHelper.queryVertexIndex(this.getGraph(MapdbGraph.class), indexedContainer.key, indexedContainer.value).stream()
                            .filter(vertex -> HasContainer.testAll(vertex, this.hasContainers))
                            .collect(Collectors.<Vertex>toList()).iterator();
    }

    private HasContainer getIndexKey(final Class<? extends Element> indexedClass) {
        final Set<String> indexedKeys = this.getGraph(MapdbGraph.class).getIndexedKeys(indexedClass);
