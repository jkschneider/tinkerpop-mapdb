...
public class MapdbIndex<T extends Element> {
    ...
    public MapdbIndex(final MapdbGraph graph, final Class<T> indexClass) {
        this.graph = graph;
        this.index = graph.db.createHashMap(graph.getName() + "-index-" + instanceCount.incrementAndGet()).make();
